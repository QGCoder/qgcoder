...
static CANON_MOTION_MODE _motion_mode = CANON_CONTINUOUS;
   ...
   /* qgcoder: was [100]. qgcoder keeps the parameter file in the user's data
   directory, whose absolute path can easily be longer than that. 256 is
   RS274NGC_TEXT_SIZE, the size of every buffer this name is copied into, and
   GET_EXTERNAL_PARAMETER_FILE_NAME still refuses to overrun a shorter one. */
char                     _parameter_file_name[256];
#ifdef AA
...
void INIT_CANON()
{
   /* qgcoder: the interpreter is linked into the application instead of being
   run as a separate process, so this dummy world model has to be returned to
   its start-up state here.  Otherwise a second interpreter run would inherit
   the tool, units and position left behind by the previous one.  _tools and
   _tool_max are owned by the driver and are deliberately not touched. */
    _active_plane SET_TO CANON_PLANE_XY;
    _active_slot SET_TO 1;
    _feed_rate SET_TO 0.0;
    _flood SET_TO 0;
    _length_unit_factor SET_TO 1;
    _length_unit_type SET_TO CANON_UNITS_MM;
    _line_number SET_TO 1;
    _mist SET_TO 0;
    _motion_mode SET_TO CANON_CONTINUOUS;
#ifdef AA
    _probe_position_a SET_TO 0;                   /*AA*/
#endif
#ifdef BB
    _probe_position_b SET_TO 0;                   /*BB*/
#endif
#ifdef CC
    _probe_position_c SET_TO 0;                   /*CC*/
#endif
    _probe_position_x SET_TO 0;
    _probe_position_y SET_TO 0;
    _probe_position_z SET_TO 0;
#ifdef AA
    _program_origin_a SET_TO 0;                   /*AA*/
#endif
#ifdef BB
    _program_origin_b SET_TO 0;                   /*BB*/
#endif
#ifdef CC
    _program_origin_c SET_TO 0;                   /*CC*/
#endif
    _program_origin_x SET_TO 0;
    _program_origin_y SET_TO 0;
    _program_origin_z SET_TO 0;
#ifdef AA
    _program_position_a SET_TO 0;                 /*AA*/
#endif
#ifdef BB
    _program_position_b SET_TO 0;                 /*BB*/
#endif
#ifdef CC
    _program_position_c SET_TO 0;                 /*CC*/
#endif
    _program_position_x SET_TO 0;
    _program_position_y SET_TO 0;
    _program_position_z SET_TO 0;
   /* these three have no initialiser above, so a fresh process starts them
   at zero; reproduce that rather than picking a "nicer" value. */
    _spindle_speed SET_TO 0;
    _spindle_turning SET_TO 0;
    _traverse_rate SET_TO 0;
}
